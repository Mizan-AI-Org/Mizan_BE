    ...
    async handleClockIn(event: any) {
        ...
        if (details?.latitude && details?.longitude) {
            ...
            if (result.success || result.status === 'success') {
                ...
                // Trigger checklist flow
                try {
                    console.log(`📋 Automatically triggering checklist flow for ${staffName}`);

                    const initResult = await this.apiService.initiateChecklistForAgent(staffId);

                    if (initResult.status === 'started' || initResult.status === 'restored') {
                        const { current_step, checklist } = initResult;

                        await this.apiService.sendWhatsapp({
                            phone: phone,
                            type: 'text',
                            body: `📋 *Shift Checklist: ${checklist.name}*\n\nYour first item:\n\n*${current_step.title}*\n\nPlease reply with *YES*, *NO*, or *N/A*.`
                        }, agentKey);
                    } else if (initResult.status === 'completed') {
                        // Checklist already done
                        console.log(`Checklist already completed for ${staffName}`);
                    }
                } catch (checklistError: any) {
                    console.error("Failed to auto-trigger checklist:", checklistError.message);
                }
