...
export default class ScheduleOptimizerTool {
    constructor(apiService) {
        this.name = "schedule_optimizer";
        this.description = "Optimize staff schedules based on predicted demand and staff availability.";
    ...
    async execute(input, context) {
        ...
        try {
            ...
            return {
                ...
                schedule_summary: result.shifts.map((s) => `${s.date} ${s.time}: ${s.staff} (${s.role || 'Staff'})`).slice(0, 10) // Limit summary
            };
        }
        catch (error) {
