...
const staffManagementWebhook = new LuaWebhook({
    ...
    execute: async ({ query, headers, body }) => {
        ...
        for (let attempt = 1; attempt <= 3; attempt++) {
            try {
                result = await Data.create('staff-management-events', eventData, `${body.eventType} ${body.staffName} ${body.role}`);
                break;
            }
            catch (err) {
                console.error(`Attempt ${attempt}: Failed to store staff event`, err);
                if (attempt === 3)
                    throw new Error('Failed to store staff event after 3 attempts');
