...
const forecastingWebhook = new LuaWebhook({
    ...
    execute: async ({ query, headers, body }) => {
        ...
        for (let attempt = 1; attempt <= 3; attempt++) {
            try {
                result = await Data.create('forecasting-events', eventData, `${body.eventType} ${body.forecastPeriod.startDate} confidence:${(body.confidence * 100).toFixed(0)}%`);
                break;
            }
            catch (err) {
                console.error(`Attempt ${attempt}: Failed to store forecasting event`, err);
                if (attempt === 3)
                    throw new Error('Failed to store forecasting event after 3 attempts');
                await new Promise(r => setTimeout(r, attempt * 500));
            }
        }
        console.log(`✅ Forecast processed: ${result.id}`);
        // Calculate business impact
        const potentialSavings = body.inventoryAlerts?.reduce((sum, item) => sum + (item.currentStock > item.reorderPoint ? 50 : 0), 0) || 0;
