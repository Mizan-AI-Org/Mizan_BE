...
export class ParseDocumentTool implements LuaTool {
    name = "parse_document";
    description =
        "Read a non-image document the manager attached (PDF, Word/.docx, Excel/.xlsx, CSV, plain text) and act on it. Use this whenever the user uploads ANY non-image file — do NOT call parse_photo on docs. The backend extracts text and classifies the doc; if it's a supplier invoice with confident vendor/amount/due_date it logs it automatically and returns the new invoice id. If the classifier is not confident, OR any required field is missing, the response will tell you to ASK the user for those fields — do that, then call record_invoice with the values they confirm. NEVER fabricate vendor / amount / invoice_number / due_date / issue_date.";
