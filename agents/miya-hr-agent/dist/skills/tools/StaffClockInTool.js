...
const optionalCoord = z.preprocess((v) => (v === null || v === undefined || v === "" ? undefined : v), z.union([z.number(), z.string()]).optional());
 ...
 */
export default class StaffClockInTool {
    constructor(apiService) {
        this.name = "staff_clock_in";
        this.description = "Clock in a staff member by their phone (WhatsApp is the canonical staff attendance channel — everyone clocks in/out there; this tool backs that when you must write the event from chat). Use when staff say 'clock in', 'clock-in', 'I want to clock in', etc. Pass their phone from WhatsApp context; if they just shared their location, pass latitude and longitude for geofence validation. A scheduled shift is not required — unplanned clock-ins are allowed when location checks pass. Staff with multiple shifts in a day still get the right shift linked when they clock in within the usual window. ALWAYS reply to the staff with the exact 'message' field returned by this tool, character-for-character — that string is already in the staff's language and is the source of truth. NEVER substitute your own apology like 'something went wrong' or 'please try again in a moment' — those are not what the backend said.";
        this.inputSchema = z
            .object({
            phone: z
                .preprocess((v) => (v === undefined || v === null ? "" : String(v)), z.string())
                .describe("Sender phone if known; may be empty — the tool resolves WhatsApp/channel uid (e.g. whatsapp:+212…) automatically."),
            ...
            location: locationAttachmentSchema.describe("Optional nested location object from WhatsApp/Meta when coordinates are not top-level."),
        })
            .passthrough();
     ...
     */
    mapErrorToCode(rawError) {
        const e = String(rawError || "").toLowerCase();
        if (!e)
            return "server_error";
        if (e.includes("location required"))
            return "location_required";
        if (e.includes("outside geofence"))
            return "outside_geofence";
        if (e.includes("geofence not configured"))
            return "no_geofence";
        if (e.includes("no shift") || e.includes("no scheduled shift"))
            return "no_shift";
        if (e.includes("invalid coordinates"))
            return "invalid_coordinates";
        if (e.includes("invalid or missing phone"))
            return "no_phone";
        if (e.includes("staff not found"))
            return "no_phone";
        if (e.includes("unauthorized") || e.includes("agent key"))
            return "unauthorized";
    ...
    async execute(rawInput) {
        ...
        if (!parsed.success) {
            ...
            return {
                status: "error",
                code: "invalid_input",
                message: "We couldn't read your location from this message. Please tap Share Location / Current Location in WhatsApp, or try again.",
            };
        }
        ...
        const hasAgentKey = !!(env("LUA_WEBHOOK_API_KEY") ||
            env("WEBHOOK_API_KEY") ||
        ...
        if (!hasAgentKey) {
            ...
            return {
                status: "error",
                code: "unauthorized",
                message: "Clock-in is temporarily unavailable (the agent isn't authorised to talk to the backend). Please contact your manager.",
            };
        }
        const phone = resolveStaffPhoneForByPhoneTools({ uid: user.uid, data: userData, _luaProfile: profile }, input.phone);
        if (!phone || phone.length < 6) {
            console.error(`[StaffClockInTool] No valid phone: input=${input.phone}, uid=${user.uid}`);
            return {
                status: "error",
                code: "no_phone",
        ...
        try {
            const { lat: latN, lng: lngN } = latLngFromInput(input);
            console.log(`[StaffClockInTool] Calling clock-in-by-phone (phone=${phone.slice(-4).padStart(phone.length, "*")}, lat=${latN ?? "-"}, lng=${lngN ?? "-"})`);
            ...
            console.log(`[StaffClockInTool] Result: success=${result.success}, error=${result.error || "-"}, message_for_user="${(result.message_for_user || "").slice(0, 120)}"`);
            if (result.success) {
                return {
                    status: "success",
                    code: result.already_clocked_in ? "already_clocked_in" : "clocked_in",
            ...
            const message = result.message_for_user ||
                result.error ||
            ...
            if (code === "location_required") {
                return {
                    ...
                    message,
                    miya_directive: "This is the expected next step — NOT an error. Relay the message field VERBATIM. " +
            ...
            return {
                ...
                code,
                message,
                miya_directive: "Relay the message field VERBATIM to the staff. Do NOT substitute a generic apology " +
            ...
            if (/Buffer|ArrayBuffer|first argument must be of type string/i.test(em)) {
                ...
                return {
                    status: "error",
                    code: "network_error",
                    message: "We couldn't finish processing your clock-in. Please try sharing your location again in a moment, or contact your manager.",
                };
            }
            console.error(`[StaffClockInTool] Unexpected error: ${error?.message || error}`, error?.response?.data ? JSON.stringify(error.response.data).slice(0, 300) : "");
            const apiBody = error?.response?.data || {};
            return {
                status: "error",
                code: error?.response ? "server_error" : "network_error",
                message: apiBody.message_for_user ||
