...
export default class StaffClockOutTool {
    constructor(apiService) {
        this.name = "staff_clock_out";
        this.description = "Clock out a staff member by their phone (WhatsApp is the canonical attendance channel — staff clock out there; this tool backs Miya-mediated chat). Use when staff say 'clock out', 'clock-out', 'I want to clock out', etc. Pass their phone from WhatsApp context. Reply with the exact message_for_user returned.";
    ...
    async execute(input) {
        ...
        try {
            ...
            return { status: "error", message: error.response?.data?.message_for_user || "I encountered an issue. Please try again or contact your manager." };
        }
    }
}
