...
export function resolveMizanUserIdFromUser(user) {
    if (!user)
        return undefined;
    const userData = user.data || {};
    const profile = user._luaProfile || {};
    const metadata = profile.metadata && typeof profile.metadata === "object" ? profile.metadata : {};
    const asStr = (v) => typeof v === "string" && v.trim().length > 0 ? v.trim() : undefined;
    const ordered = [
    ...
    if (typeof sessionId === "string") {
        const su = sessionId.match(/(?:^|[-_])user-([0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})/i);
        if (su?.[1])
            return su[1];
    }
    const luaId = user.id;
 ...
 */
export async function resolveAgentContext(inputRestaurantId) {
    const user = await User.get();
    const userData = user ? (user.data || {}) : {};
    const profile = user ? (user._luaProfile || {}) : {};
    const metadata = profile.metadata && typeof profile.metadata === "object" ? profile.metadata : {};
    // --- Restaurant ID ---
    let restaurantId = inputRestaurantId ||
        user?.restaurantId ||
    ...
    if (!restaurantId) {
        const sessionId = metadata.sessionId ||
            ...
            userData.sessionId ||
            user?.sessionId ||
            user?.uid; // Lua may use uid for web sessions
    ...
    if (!restaurantId) {
        const rc = profile.runtimeContext || user?.runtimeContext || userData.runtimeContext;
    ...
    if (!restaurantId && user?.context) {
        const ctx = String(user.context);
    ...
    if (!restaurantId) {
        ...
        if (userToken && typeof userToken === "string" && userToken.length > 50) {
            try {
                ...
                console.warn("[agentContext] agent-context fallback failed:", e?.message);
            }
        }
    }
    ...
    const phoneFromData = userData.phone ?? profile.phoneNumber ?? profile.mobileNumber ?? metadata.phone;
    const rawPhone = [phoneFromData, phoneFromUid].find((p) => p && String(p).replace(/[^0-9]/g, "").length >= 6);
    ...
    if (!restaurantId && agentKey && user) {
        ...
        if (phone)
            body.phone = phone;
        const emailGuess = (typeof userData.email === "string" && userData.email.trim()) ||
        ...
        if (Object.keys(body).length > 0) {
            try {
                ...
                console.warn("[agentContext] resolve-tenant failed:", e?.message);
            }
        }
    }
