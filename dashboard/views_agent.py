...
def _resolve_task_category(*, raw: object, title: str, description: str) -> str | None:
    ...
    if raw is not None and str(raw).strip():
        ...
        aliases = {
            ...
            "EVENT": "MEETING", "EVENTS": "MEETING", "EVENEMENT": "MEETING",
            "RUBRIQUE": "MEETING", "SECTION": "MEETING",
            "DOCUMENTS": "DOCUMENT",
...
def agent_reassign_dashboard_task(request):
    ...
    Change the assignee on an existing ``dashboard.Task`` and WhatsApp the
    new assignee (same behaviour as PATCH …/tasks-demands/<id>/assignee/).

    Body: ``task_id`` (UUID), exactly one assignee field (``user_id`` /
    ``email`` / ``phone`` / ``name``), optional ``notify_whatsapp`` (default
    true), optional ``whatsapp_message`` (replaces default WA body),
    optional ``note`` (appended to WA body).
    """
    from dashboard.api.tasks_demands import _notify_dashboard_task_new_assignee
    from scheduling.views_agent import _resolve_restaurant_for_agent, _try_jwt_restaurant_and_user
    ...
    try:
        ...
        data = request.data if isinstance(getattr(request, "data", None), dict) else {}
        raw_tid = _get_first(data, "task_id", "taskId", "id")
        if not raw_tid:
            return Response(
                {
                    "success": False,
                    "error": "task_id required",
                    "message_for_user": "I need the task id to reassign it.",
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            task = Task.objects.select_related("assigned_to").get(
                pk=raw_tid, restaurant=restaurant
            )
            ...
            return Response(
                {
                    "success": False,
                    "error": "Not found",
                    "message_for_user": "That task was not found in your workspace.",
                },
                status=status.HTTP_404_NOT_FOUND,
            )
            ...
            return Response(
                {
                    ...
                    "error": assignee_err or "Assignee not found",
                    "message_for_user": assignee_err
                    or "I couldn't find that team member in this workspace.",
                },
                status=status.HTTP_404_NOT_FOUND,
            )
        ...
        if old_assignee and old_assignee.id == assignee.id:
            assignee_display = (
                f"{(assignee.first_name or '').strip()} {(assignee.last_name or '').strip()}".strip()
                or assignee.email
            )
            return Response(
                {
                    ...
                    "message_for_user": f"'{task.title}' is already assigned to {assignee_display}.",
                },
                status=status.HTTP_200_OK,
            )

        note = str(_get_first(data, "note") or "").strip()
        notify_whatsapp = _coerce_bool(
            _get_first(data, "notify_whatsapp", "notifyWhatsapp", "send_whatsapp"),
            default=True,
        )
        wa_override = _get_first(data, "whatsapp_message", "whatsappMessage", "message")
        if isinstance(wa_override, str):
            wa_override = wa_override.strip() or None
        else:
            wa_override = None
        ...
        wa_result = _notify_dashboard_task_new_assignee(
            task=task,
            new_assignee=assignee,
            acting_user=acting_user,
            note=note,
            had_previous_assignee=old_assignee is not None,
            whatsapp_override=wa_override,
            send_whatsapp=notify_whatsapp,
        )
        ...
        if wa_result.get("sent"):
            wa_phrase = "WhatsApp notification sent."
        elif wa_result.get("skipped_reason") == "disabled":
            wa_phrase = "WhatsApp skipped as requested."
        elif wa_result.get("skipped_reason") == "no_phone":
            wa_phrase = "No phone on file — they'll see the in-app notification only."
        elif wa_result.get("error"):
            wa_phrase = "WhatsApp could not be delivered; the reassignment is saved in Mizan."
        else:
            wa_phrase = ""

        message_for_user = (
            f"Reassigned '{task.title}' to {assignee_display}. {wa_phrase}"
        ).strip()
        ...
        return Response(
            {
                "success": False,
                "error": str(exc)[:200],
                "message_for_user": "Something went wrong while reassigning that task. Please try again.",
            },
            status=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
